...
public final class DosViewReaderTest
    extends ViewReaderTest<DosViewReader, DosFileAttributeView>
{
    @SuppressWarnings("UnsecureRandomNumberGeneration")
    private final Random random = new Random();
    private DosFileAttributes attrs;

    public DosViewReaderTest()
    {
        super(DosFileAttributeView.class, "lastModifiedTime",
    ...
    protected void initReader()
        throws IOException
    {
        attrs = mock(DosFileAttributes.class);
        when(view.readAttributes()).thenReturn(attrs);
        reader = new DosViewReader(view);
    }
    ...
    public void readReadOnlyTest()
        throws IOException
    {
        reader.getAttributeByName("readonly");

        verify(view, only()).readAttributes();
        verify(attrs, only()).isReadOnly();
    }

    @Test(dependsOnMethods = "readReadOnlyTest")
    public void readReadOnlySuccessTest()
        throws IOException
    {
        final boolean readonly = true;

        when(attrs.isReadOnly()).thenReturn(readonly);

        final Object actual = reader.getAttributeByName("readonly");

        assertThat(actual).isEqualTo(readonly);
    }
    ...
    public void readHiddenTest()
        throws IOException
    {
        reader.getAttributeByName("hidden");

        verify(view, only()).readAttributes();
        verify(attrs, only()).isHidden();
    }

    @Test(dependsOnMethods = "readHiddenTest")
    public void readHiddenSuccessTest()
        throws IOException
    {
        final boolean hidden = true;

        when(attrs.isHidden()).thenReturn(hidden);

        final Object actual = reader.getAttributeByName("hidden");

        assertThat(actual).isEqualTo(hidden);
    }
    ...
    public void readSystemTest()
        throws IOException
    {
        reader.getAttributeByName("system");

        verify(view, only()).readAttributes();
        verify(attrs, only()).isSystem();
    }

    @Test(dependsOnMethods = "readSystemTest")
    public void readSystemSuccessTest()
        throws IOException
    {
        final boolean system = true;

        when(attrs.isSystem()).thenReturn(system);

        final Object actual = reader.getAttributeByName("system");
    ...
    public void readArchiveTest()
        throws IOException
    {
        reader.getAttributeByName("archive");

        verify(view, only()).readAttributes();
        verify(attrs, only()).isArchive();
    }
