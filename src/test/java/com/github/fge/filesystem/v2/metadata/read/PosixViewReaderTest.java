...
public final class PosixViewReaderTest
    extends ViewReaderTest<PosixViewReader, PosixFileAttributeView>
{
    @SuppressWarnings("UnsecureRandomNumberGeneration")
    private final Random random = new Random();
    private PosixFileAttributes attrs;

    public PosixViewReaderTest()
    {
        super(PosixFileAttributeView.class, "lastModifiedTime",
    ...
    protected void initReader()
        throws IOException
    {
        attrs = mock(PosixFileAttributes.class);
        when(view.readAttributes()).thenReturn(attrs);
        reader = new PosixViewReader(view);
    }
        ...
        throws IOException
    {
        ...
        when(attrs.owner()).thenReturn(owner);
    ...
    public void readGroupTest()
        throws IOException
    {
        reader.getAttributeByName("group");

        verify(view, only()).readAttributes();
        verify(attrs, only()).group();
    }

    @Test(dependsOnMethods = "readGroupTest")
    public void readGroupSuccessTest()
        throws IOException
    {
        final GroupPrincipal group = mock(GroupPrincipal.class);

        when(attrs.group()).thenReturn(group);

        final Object actual = reader.getAttributeByName("group");

        assertThat(actual).isSameAs(group);
    }
    ...
    public void readPermissionsTest()
        throws IOException
    {
        reader.getAttributeByName("permissions");

        verify(view, only()).readAttributes();
        verify(attrs, only()).permissions();
    }

    @Test(dependsOnMethods = "readPermissionsTest")
    public void readPermissionsSuccessTest()
        throws IOException
    {
        @SuppressWarnings("unchecked")
        final Set<PosixFilePermission> permissions = mock(Set.class);

        when(attrs.permissions()).thenReturn(permissions);

        final Object actual = reader.getAttributeByName("permissions");

        assertThat(actual).isSameAs(permissions);
    }
